...
router.delete("/portifolio/:id", auth, async (req, res) => {
  try {
    ...
    if (!projeto) {
      return res
        .status(404)
        .json({ message: "Projeto não encontrado para deletar" });
    }
...
router.put("/portifolio/:id", auth, async (req, res) => {
  try {
    ...
    const result = await dbConn
      .collection("projetos")
      .updateOne({ _id: new ObjectId(id) }, { $set: { nameProject } });

    if (result.modifiedCount === 0) {
      return res
        .status(404)
        .json({ message: "Projeto não encontrado ou nome já era igual" });
    }
...
router.put("/portifolio/:id/description", auth, async (req, res) => {
  try {
    ...
    if (description === undefined) {
      return res
        .status(400)
        .json({ message: "Campo 'description' é obrigatório." });
    }
    ...
    res.status(500).json({ message: "Erro interno ao atualizar descrição." });
  }
});
...
router.delete("/portifolio/:projectId/imagem", auth, async (req, res) => {
  try {
    ...
    if (!imageUrl) {
      return res
        .status(400)
        .json({ message: "Informe o URL da imagem para apagar" });
    }
    ...
    if (!image) {
      return res
        .status(404)
        .json({ message: "Imagem não encontrada no projeto" });
    }
